...
static void spi_master_init()
{
	...
	if (ESP_ERR_INVALID_STATE == ret)
		ESP_LOGW(TAG, "El spi_bus posiblemente ya esté inicializado.");
	else
		assert(ret==ESP_OK);
...
{
	if (fontIndex < fonts_count) // te olvidaste de poner el tama�o correcto en InitFonts?
		dev.font.fx = &fonts[fontIndex];
}

/*
 * setea la font a usar y retorna un puntero a caracteristicas de la font (tama�o, etc)
...
/*
 * Setea la posicion donde imprimir� lcdPrintf
...
/*
 * Imprime el texto en las posiciones que sete� lcdSetCursor
 */
